...
export function EnrollmentDialog({ open, camp, registration, students, onSave, onClose }: EnrollmentDialogProps) {
  ...
  const removeStudent = (id: string) => {
    ...
    setFriendGroups((prev) => prev.map((g) => g.filter((sid) => sid !== id)).filter((g) => g.length >= 2));
  };

  const toggleNewGroupMember = (id: string) => {
    setNewGroupIds((prev) => (prev.includes(id) ? prev.filter((s) => s !== id) : [...prev, id]));
  };
  ...
  const handleSave = () => {
    const validGroups = friendGroups.map((g) => g.filter((id) => selectedIds.includes(id))).filter((g) => g.length >= 2);
    onSave({
  ...
  const instancesNeeded = selectedIds.length > 0 ? Math.ceil(selectedIds.length / camp.maxSize) : 0;
      ...
      <DialogContent dividers>
                  ...
                  .map((student) => (
                    <TableRow key={student.id}>
                      ...
                      <TableCell align="right">
                        ...
                        <IconButton size="small" onClick={() => removeStudent(student.id)} color="error" title="Remove from class">
        ...
        {editingStudentId && (
          <Dialog open={true} onClose={() => setEditingStudentId(null)}>
            <DialogTitle>Edit Friend Groups for {students.find((s) => s.id === editingStudentId)?.firstName}</DialogTitle>
            <DialogContent>
                ...
                {friendGroups.length === 0 ? (
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    {friendGroups.map((group, i) => (
                      <FormControlLabel
                        key={i}
                        control={<Checkbox checked={group.includes(editingStudentId)} onChange={() => toggleStudentGroup(editingStudentId, i)} />}
                        label={
                          <Typography variant="body2">
                            Group {i + 1}: {group.map((id) => studentName(id)).join(', ')}
                          </Typography>
                        }
                      />
                    ))}
